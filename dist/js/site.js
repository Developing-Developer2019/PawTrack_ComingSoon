...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (!String.prototype.padStart) {
    String.prototype.padStart = function (len, ch = " ") {
      return (ch.repeat(len) + this).slice(-len);
    };
  }

  /**
   * Initialize countdown timer - Safari Mobile Compatible
   */
  function initializeCountdown() {
    try {
      // Safari-compatible date format - use explicit Date constructor
      const launchDate = new Date(2025, 10, 22, 10, 0, 0, 0); // Note: month is 0-indexed (10 = November)
      ...
      function updateCountdown() {
        try {
          ...
          if (distance > 0) {
            ...
            // Clear interval and set to zero
      ...
      function updateCountdownDisplay(elementId, value) {
        ...
        if (element) {
          ...
          element.textContent = z(numValue);
        } else {
        }
      }

      function setDefaultCountdown() {
        // Fallback: show static countdown or error message
        updateCountdownDisplay("days", 0);
        updateCountdownDisplay("hours", 0);
        updateCountdownDisplay("minutes", 0);
        updateCountdownDisplay("seconds", 0);
  ...
  `;
    ...
    const newToast = document.querySelector(
      ".toast-container:last-child .toast"
    );
